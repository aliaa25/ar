...
export default function page() {
  ...
  // 🟢 إضافة state للتحقق من Mobile
  const [isMobile, setIsMobile] = useState(false);

  // 🟢 useEffect للتحقق من Mobile
  useEffect(() => {
    const checkMobile = () => {
...
function projectToScreen(vec, camera) {
  ...
  if (!vec || !camera) {
    console.warn('Invalid parameters for projectToScreen');
    return { x: 0, y: 0 };
  }
  ...
  try {
    ...
    // Validate projected coordinates before calculations
    if (!isFinite(projected.x) || !isFinite(projected.y)) {
      console.warn('Invalid projected coordinates:', projected);
      return { x: 0, y: 0 };
    }
    ...
    // Final validation of calculated screen coordinates
    const safeX = isFinite(x) ? x : 0;
    const safeY = isFinite(y) ? y : 0;
    
    return { x: safeX, y: safeY };
  } catch (error) {
    console.error('Error in projectToScreen:', error);
    return { x: 0, y: 0 };
  }
}
  
  // Update dimension popup container to follow the model.
 // Fixed useEffect for dimension popup positioning
useEffect(() => {
  let animId;
    ...
    if (showDimensionPopup && selectedModelId) {
      ...
      if (modelEl && cameraObj) {
        try {
          ...
          if (box.isEmpty()) {
            console.warn('Empty bounding box for model:', selectedModelId);
            return;
          }
          ...
          // Validate topCenter coordinates
          if (!isFinite(topCenter.x) || !isFinite(topCenter.y) || !isFinite(topCenter.z)) {
            console.warn('Invalid topCenter coordinates:', topCenter);
            return;
          }
          ...
          if (!isFinite(proj.x) || !isFinite(proj.y)) {
            console.warn('Invalid projection results:', proj);
            return;
          }
          
          // Position the container slightly above the top-center with validation
          const safeLeft = Math.max(0, Math.min(proj.x, window.innerWidth - 200)); // Prevent overflow
          const safeTop = Math.max(0, proj.y - 40); // Ensure positive values
            ...
            if (!meshBox.isEmpty()) {
              ...
              if (isFinite(width) && isFinite(height) && isFinite(depth)) {
                setDimensionsText({
                  ...
                  depth: `Length: ${depth.toFixed(2)}`,
                });
              }
            }
          }
        } catch (error) {
          console.error('Error updating dimension popup position:', error);
        }
      }
    }
    ...
    >
      ...
      {modelSrc ? (
        <a-scene embedded physics className="w-full h-full rounded-lg shadow-lg">
          ...
          {/* 🟢 الكاميرا مع التحكم الشرطي في الكيرسور */}
          <a-camera
            position="0 1.6 4"
            scale="4.5 4.5 4.5"
            look-controls="mouseEnabled: true; reverseTouchDrag: false;"
            wasd-controls="enabled: false"
          >
            {/* 🟢 إظهار الكيرسور فقط على الـ Desktop */}
            {!isMobile && (
              <a-cursor
                raycaster="objects: .clickable-item, .clickable-floor;"
                fuse="true"
